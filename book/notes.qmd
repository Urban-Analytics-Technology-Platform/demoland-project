...
The good thing of the OS data is that they are accurate (resolution, categories) AND they contain also the access points (which for everyone's sake we are gonna refer to as _entrances_).<br/>
**NOTE:** we need _entrances_ to green space as the destinations for our time travel matrices.<br/>
A good candidate to make up for not covered areas is the _landuse_ layer from OpenStreetMap (OSM).<br/>

[from OSM landuse layer we consider the tags/fields: `cemetery`, `forest`, `nature reserve`, `park`, `recreation ground`]<br/>

OSM data has very much good cover of the existing green sites, but: lacks a proper classification, doesn't have entrances to sites, doesn't distinguish between public/private/military spaces, therefore the use of OSM would bring to _over_-estimate green spaces.
Example: in OSM we have some side-road areas inside [Metrocentre](https://en.wikipedia.org/wiki/MetroCentre_(shopping_centre)) classified as forest.

In order to understand parks where people actually can/does pass through, a cross-check was carried out manually looking for parks with existing trails (passing through them) using [https://hiking.waymarkedtrails.org/](https://hiking.waymarkedtrails.org/) AND Google Street View to check entrances to parks/greens/ etc [IE some entrances actually show a sign that make it clear if it's a public access or not]

List of parks/green spaces which we would want to consider in the green belt/suburbs of Tyne and Wear (and their status, IE open/public etc):<br/>

| park name | status |location |
|-|:-:|-|
|Chopwell Woodland | open | SW |
| Milkweelburn Wood | open | SW|
| Watergated Park | open |  |
| Silverhill Wood / Ravesworth Estate | closed |  |
| Banesley Lane woodland | open |  |
| Hedley Hall Woods | open |  |
| Spen Banks / Sherburn Green Woods | open |  |
| Clinty Wood | ? |  |
| Gibside national Trust | closed ($$) |  |
...
In order to do so, we need to clean the data from the eccessive entrances: many sites are included within other ones (for example a playground or tennis court can be enclosed in a public park). These enclosed sites also generate an overestimate of the accessible area, because they are overlapping with the main major park - which is the only category/feature/polygon we would like to consider.<br/>

First of all we clean the OS data from not-wanted categories, IE areas which are green but not openly accessible by the public.<br/>
These include: allottments, golf courses, bowling greens.
...
The hierarchy of the layers seems to be roughly this:<br/>
`Public Park Or Garden` _contains_<br/>
`Playing Field` _contains_<br/>
`Other Sport Facilities` _contains_<br/>
rest of categories.<br/>

After several trials with spatial joins and other geospatial operations, have managed to obtain a good results by separating the above categories into single layers and performing operations among them.<br/>
See notebook `greenspace_datacleaning.ipynb` for details.
...
- ttm generation:
     ...
     - can generate one single table with multimodal trip > _can create a comprehensive table using scripting_
     ...
     - using random date and time in database: `2023,1,19,8,30`;
         ...
         - optimal date? ([Journey Time Statistics](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/853603/notes-and-definitions.pdf) use a _Tuesday in October_)
...
Two screenshots from sources used to cross-check the data:<br/>
1. Dustin's bus spotting website ([bus spotting](https://dabreegster.github.io/bus_spotting/))
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/gtfs_bus_spotting.png?raw=true)

2. personal map on [Felt](https://felt.com/map/TyneWear-sXwTHCjIQLGJGDS0dYAF9AD) with added transport network layer
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/gtfs_felt.png?raw=true)


### Jobs location

We have number of jobs as a datum down to LSOA level, it comes from [nomis](https://www.nomisweb.co.uk/sources/bres)

<blockquote> An employer survey of the number of jobs held by employees broken down by full/part-time and detailed industry (5 digit SIC2007). The survey records a job at the location of an employees workplace. Available from country down to lower level super output area and Scottish datazone.</blockquote>
...
See as an example the image below (created in Qgis) showing the Team Valley trading estate in Gateshead [location](https://goo.gl/maps/zsKWLGDe5jDH1pRJ7)

![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/compare_LSOAvsWPZ.png?raw=true)
Red dots = OA centroids (our origins)<br/>pink lines = LSOAs boundaries<br/>
yellow dots = LSOAs centroids <br/>blue dots = WPS centroids

Looking for alternatives, I have found a geography for the Census that takes into account the working population.

[Workplace Zones (WPZ)](https://www.ons.gov.uk/methodology/geography/geographicalproducts/areaclassifications/2011workplacebasedareaclassification/classificationofworkplacezonesfortheukabouttheareaclassifications)
...
The data available at this geographic level are [here](https://www.nomisweb.co.uk/sources/census_2011_wp) and they include the _Workplace population (table WP101EW)_
...
We have decided to use the number of workers as a proxy for jobs count (STRONG ASSUMPTION).<br/>
We can then use as destinations the WPZ centroids, and as n. of jobs (opportunities) the working population for the WPZ.<br/>
A stronger variable could be generated using the location from the WPZ (destination), and as opportunities a interpolation from the LSOA jobs count to the WPZ areas.<br/>
Note that the relation between Output Areas and WPZ is that "in England and Wales they [WPS] are designed to align with Middle Layer Super Output Areas (MSOAs)".


**Conclusions:** In any case we need to make an approximation: using LSOA we give up on location, using WPZ we give up on actual (estimated?) job count.<br/>

The ideal situation would be to have the number of jobs and their location / employment centres.<br/>
A way to deal with the lack of data on this, could be to generate it using [Free Company Data Product](http://download.companieshouse.gov.uk/en_output.html)

> downloadable data snapshot containing basic company data of live companies on the register

From this data set we can get number of companies registered _at postcode level_ (see data fields [here](https://resources.companieshouse.gov.uk/toolsToHelp/pdf/freeDataProductDataset.pdf)) which is quite detailed, and then find a way to aggregate/interpolate them to identify “job centres”.<br/>
We have postcodes centroids from geoportal-gov.
...
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/tynewear_houseprices2022_greenspace_lsoa.png?raw=true)
*Note:*
...
you can run and play around with `accessibility`, a [package](https://github.com/ipeaGIT/accessibility) in R from Rafa Pereira from [ipeagit](https://ipeagit.github.io/accessibility/articles/accessibility.html)
...
In order to run these functions we need to create/have the following input files:
 ...
 - supply / opportunity:
     ...
     - b. _other type of opportunity_ (ex: n of groceries, n of schools, etc... extension of greenspace? <mark>**n of access points?**</mark>)

<br/><br/> as a proxy for future complicated stuff and to try something for now, let's generate the number of access points (to greenspaces) per LSOA... <br/>what would be the fastest way:
...
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/tynewear_isochrones_withOTP.png?raw=true)*Examples of 15-90 minisochrones from (54.96835, -1.60778) generated using OTP for walk/transit mode, at 9am on 01-19-2-23)*
...
- [X] time travel matrix   --> more editable?? _shorter to generate, no need of a graph building_
     - [X] find GTFS data
     - [X] download OSM data
...
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/tynewear_lads_msoas.png?raw=true)*The 5 LADs in Tine and Wear (labelled by `LAD20NM` and `LAD20CD`), highlighted in gray the major urban areas, bordered in blue the 145 MSOAs*
...
![](https://github.com/urban-analytics-technology-platform/demoland-project/blob/main/images/rlur_example.png?raw=true)
...
- See available datasets for House Price Statistics for Small Areas (HPSSAs) from [here: houseprices_available-data_HPSSA.xlsx](https://github.com/urban-analytics-technology-platform/demoland-project/tree/main/data)
...
- filtering just the relevant grographies for Tyne and Wear county: $\rightarrow$  <mark>automate this??</mark>
     - MSOAs: [list in MSOAs_Tyne-Wear.txt](https://github.com/urban-analytics-technology-platform/demoland-project/tree/main/data)<br/>
     `MSOA20CD` = `MSOA code`<br/>
     `MSOA20NM` = `MSOA name`<br/>
     - LSOAs: [list in TyneWear_LADs_list.csv](https://github.com/urban-analytics-technology-platform/demoland-project/tree/main/data) filter by Local authority:<br/>
...
- see [Issue](https://github.com/urban-analytics-technology-platform/demoland-project/issues/5) on GitHub for relevant literature on indicators, definitions, methodology to generate UGS maps
...
| actual (X) VS estimated (O)| Indicator | Definition |Data source | Link |
|:-:|-|-|-|-|
|O| ~~Mobility emissions~~ | | ~~gov uk data estimates <br/>*at LAD level* (only)~~ |  |
|X| Air pollution hazard | | Local Authority + European data |  |
...
| n. available jobs |  |  | X | X | Nomis BRES |
| distance to employment center |  |  | X | generated |
| access to public transport |  |  | X | generated |
