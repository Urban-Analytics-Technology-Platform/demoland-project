...
- [ ] Identify the shortest path between two locations using the OSM transport network:
     ...
     - [OpenTripPlanner](https://wiki.openstreetmap.org/wiki/OpenTripPlanner) open-source multimodal trip planning software system
     ...
     - build from OSM and implementing Dijkstra myslef (??? @Dustin suggestion)
...
- between areas (IE "from this LSOA I can access N schools/jobs/... located in other LSOAS")<br/>
    calculated using origin/destination matrix for centroids (pop weighted likely) and cumjulative sum of opportunities
...
2. `travel_matrix` - origin destination matrix with travel cost column (they use time rather than distance, as a cost proxy) per LSOAs (`from_id`, `to_id`, `travel_time`)
...
In order to run these functions we need to create/have the following input files:
 ...
 - supply:
     - a. _job counts per area_ can download data at LSOA level (one file per LAD) from [Nomis BRES](https://www.nomisweb.co.uk/sources/bres)
...
- [] time travel matrix $\rightarrow$ using OTP OpenTripPlanner
     - [X] find GTFS data
     - [X] download OSM data
     - [X] build graph.boj
- [ ] supply data
- [ ] run `tracc` for calculating accessibility

Following the tutorial at [https://github.com/marcusyoung/otp-tutorial](https://github.com/marcusyoung/otp-tutorial)
