...
We have two different types of indicators: while air quality and house prices are "actual data" measurements, accessibility is estimated and generated in a deterministic way, so it can't be modelled/predicted using some explanatory variables.

### Accessibility calculation

For accessibility computation, we use the same methodology both for green spaces and jobs. We consider a set of punctual origins and destinations and the time it takes to move between them on the road network, given different transport modes.<br/>
**TBC** until green spaces methodology is under definition
...
**Destinations -** Per each destination we need to provide the coordinates and  a datum that gives the _opportunity_ (or 'supply', in our case this is the land use), for example the number of jobs available (could be number of schools, health centres, other types of countable features).

For jobs count we consider as destinations the PWC of the working population per Working Place Zones (WPZ) from the 2011 Census ([data available](https://www.nomisweb.co.uk/sources/census_2011_wp) and [definition](https://www.ons.gov.uk/methodology/geography/geographicalproducts/areaclassifications/2011workplacebasedareaclassification/classificationofworkplacezonesfortheukabouttheareaclassifications)). As an opportunity measure (the job counts) we make the approximation that the number of workers equals the number of jobs available.

For green spaces accessibility we use the open data set available from [Ordnance Survey](https://beta.ordnancesurvey.co.uk/products/os-open-greenspace) (OS). In particular as a first approximation we consider the layer "Access points" from this datum. This gives the coordinates of the access points to green spaces in all Great Britain, which we take as destinations. ~~As an opportunity, we count the cumulative sum of access points reachable within a defined time threshold (see definition below).~~


---

In order to calculate the time needed to reach the destinations, we need to have a Time Travel Matrix (TTM) between origins and destinations. To build this, we can generate a graph/network starting from data for the road network and for public transportation schedules.</br>
...
> ~~Number of access points accessible by walking in 15 minutes~~


1. obtain greenspaces from OS
    - filter out not relevant categories
2. obtain "missing" areas from Corine or OSM
3. get area size
4. run ttm (15min by walking) and generate for each OA one accesspoint per reachable park
5. generate metric per each OA as [n of parks] * [park size]
...
Time table for public transport is available in England in ... _quote UK2GTFS_
We use GTFS data because more used, also for feeding it into 

- get time table for public transport > GTFS data
- get roads network > OSM data
- generate network graph > r5 (conveyal)
- generate ttm for different modes, time (day of the year/week, time of the day), multi or single time


**3. Run accessibility analysis**

- get land use data or "supply" or "opportunities" (ex location of jobs, healthcare, shopping, education, population, leisure)

- run `tracc` (Python) on the TTM
...
Newcastle suggested scenarios (email 14 Feb 23):


1. Different built development scenarios (residential/ office/ digital & tech sector)
2. Comparison with a natural open space/ bio diversity scenario
3. Comparing densification and geographical distribution of built development- could we compare the urban core ( city centre) with the neighbourhoods and rural & villages
4. Impact of land release in the green belt ( as a comparison to urban densification)

"Original" table:

| _NCC interests/Indicators_ | Air quality | House<br/>prices | Jobs acc | Greenspace<br/>acc |
|-|:-:|:-:|:-:|:-:|-|
| Accessibility | | X | X | X |
| Environmental quality | X | | | X |
| Neighbouring/clustering of land use | | | X | X |
| Housing/employment supply and imbalance | | X | X | |
| New planned sites impact | X | X | X | X |
| Vertical vs horizontal growth | X | X | X | X |
| 15min neighbourhoods | X | X | X | X |
| Urban capacity/Housing requirements | | X | | |
| Built development (competing) uses compared | X | X | X | X |
| Land left to natural open space | X | | X | X |
| Densification by 3 character areas (eg 10% uplift) | X | X | X | X |
| Peripheral green belt release for built development | X | X | X | X |


How it can relate to our indicators and variables:


### Development scenarios

New devleopment can be modelled in two ways, depending on the scale.

Small scale development
affecting a single OA will translate into a manual change of the explanatory variables
that may be affected. For example, an addition of a single large office building will
require a change in the workplace population, possibly in land cover (may not be needed),
and a slight manual change of morhometric characters. The latter may become a bit tricky
as some characters are not suitable for a ballpark change. However, a direction shall
be clear and an approximation can be made. The development will also affect the number
of jobs available in the area, per sector. The effects of competing uses (e.g different
idustries for job allocation) will have only minor effects on the indicators and make a
little sense to model.

Large scale development is a bit easier because it is in practice a transition from one
signature type to the other. That way we can sample the values from existing areas of the
same type and use them in the modelling. Land cover, population (both jobs and homes) will
still need to be manually updated, as well as an approximation of jobs and greenspaces will
been to be made. With no intentions to _design_, we may end up randomly sampling points
from within the area of change to play the role of new jobs and parks in the scenario.

### Nature and biodiversity

This one is tricky as if it does not involve a land cover change we do not have any
variables capturing the biodiversity levels. Change of the natural settings is not
something we can model. We can model change of the urbanised land to a park (e.g. brownfield)
or a change of density from more to less dense (albeit that is not very realistic) as a
switch between signature types in the other direction than in the case above.

### Densification strategies

Densification is simlar to large-scale development scenario described above. It can be
conceptualised as a change of the signature type along the signature hierarchy towards
the higher degree of urbanisation. That way we can relatively easily sample data for the
morphometric part. We will need to estimate populations and the changes of land cover.
It is assumed that densification will not involve creation on new parks so the green
space accessibility is not affected in any significant way.

### Land release

Land release in the green belt will mean a change of signature type from non-urban to
peripheral, significant change of land cover and all populations. Former can be automatised,
the latter will be manual change. This may need to involve split of Output Areas if
only a part of a large green OA will be affected, to avoid skewing the whole map. It will
also mean move of the access points to the green belt greenery but the limitations outlined
in _Development scenarios_ regarding a placement of new parks remains.